...
<ul class="cart_list product_list_widget <?php echo $args['list_class']; ?>">
	...
	<?php if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) : ?>
		...
		<?php foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) :
			...
			$product_price = get_option( 'woocommerce_tax_display_cart' ) == 'excl' ? $_product->get_price_excluding_tax() : $_product->get_price_including_tax();
			 ...
			 if ($c_cart_Cont === "Container")
			 {
					$product_price =	get_post_meta( $_product->id, '_container_price', true );
			 }
	...
	<?php else : ?>
		...
		<li class="empty"><?php echo "No products in the ". $c_cart_Cont; ?></li>
...
<?php if ( sizeof( $woocommerce->cart->get_cart() ) > 0 ) : ?>
	...
	<p class="buttons">
		<a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class="button"><?php echo 'View '. $c_cart_Cont; ?></a>
