...
class YIT_Widget_Cart extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty( $instance['title'] ) ? $c_or_c : $instance['title'], $instance, $this->id_base );
		...
		if($yit_is_header) :
			...
			<a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class="cart_control">View <?php echo $c_or_c ?></a>
			<?php else: ?>
			<a href="<?php echo $woocommerce->cart->get_cart_url(); ?>" class="cart_control cart_control_empty">Empty <?php echo $c_or_c ?></a>
			<?php endif ?>
        ...
        echo '<div class="widget_shopping_cart_content ' . ( $hide_if_empty ? 'hide_cart_widget_if_empty' : '' ) . '">
                        ...
                        <li class="empty">' . 'No products in the '. $c_or_c . '</li>
                    </ul>
		</div>';
