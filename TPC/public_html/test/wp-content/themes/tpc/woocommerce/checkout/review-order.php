...
<div id="order_review">
	...
	<table class="shop_table">
		...
		<tfoot>
			...
			<tr class="total">
				<th><strong><?php _e( 'Pay Now (50% Deposit)', 'yit' ); ?></strong></th>
				<td>
					<strong><?php
					
					 $temp_total = $woocommerce->cart->get_total(); 
					 $temp_total = strip_tags($temp_total);
					$temp_total = str_replace('<span class="amount">$','',$temp_total);
					 $temp_total = str_replace('</span>','',$temp_total);
					 $temp_total = str_replace(',','',$temp_total);
					 $temp_total = str_replace('$','',$temp_total);
			 $temp_total = strip_tags($temp_total);
					
				 $temp_total =	 substr($temp_total,5,strlen($temp_total)-5);
				  //emp_total = cint($temp_total);
					$temp_total = ($temp_total  / 2);
					 echo woocommerce_price( $temp_total );
					 ?></strong>
					<?php
